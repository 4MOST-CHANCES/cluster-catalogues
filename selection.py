...
def main():
    ...
    xmass = Catalog('xmass', xmass, base_cols=('C3ID', 'RA_X', 'DEC_X', 'z'),
                    label='Xmass', masscol='M200c')
    ...
    kwargs = dict(
        matching=[['CHANCES', chances, matchsep_chances],
    ...
    if sample == 'lowz':
        ...
        lowz = join(
            xmass.catalog, codex.catalog, join_type='outer',
            keys=join_keys,
        ...
        print(np.sort(lowz.colnames))
        lowz = Catalog(
            'lowz', lowz, label='Lowz', masscol='M200c',
    ...
    if sample == 'evolution':
        ...
        ylim = (0.5, 10)
    completeness(
        chances['z'][matches[0]], chances['MSZ'][matches[0]],
        psz['REDSHIFT'], psz['MSZ'], zbins, mbins, 
        f'chances_{sample}', 'psz', sample, ylim=ylim, ylabel='MSZ')
...
def load_splus(chances):
    ...
    for name, ra, dec, in_splus, notes \
            in chances['name','ra','dec','SPLUS','SPLUS comments']:
        ...
        splus.add_row(
            [f'{name}_{semester}', '--', '--', 2000, c, ra, dec])
    return splus
...
def plot_xmass_codex(xmass, codex):
    ...
    # Xmass
    kwargs = dict(c=xmass['lambda'], s=50, edgecolors='k',
                  cmap='inferno_r', norm=LogNorm(vmax=100), alpha=0.8)
    ax = axes[0]
...
def selfunc(args, cat, ref_level=0.9, Nref=100,
    ...
    plt.colorbar(im, ax=ax, label=f'$N(<M_\mathrm{{{mlabel}}}|z)$ / $N(z)$',
                 fraction=0.05, pad=0)
    if 'max' in ref_level:
        c = 'w'
    else:
    ...
    for zi, Nz_i in zip(z0_analysis, Nz):
        ax.annotate(f'{Nz_i:.0f}', xy=(zi,ytext), ha='center', va='center',
                    fontsize=12)
    if args.sample == 'evolution':
        ax.set(xlim=(0, 0.5))
    if args.sample == 'evolution':
    ...
    shapley, horologium = supercluster_mask(
        ...
        horologium_kwargs=dict(
            edgecolor='C6', facecolor='none', lw=3, zorder=-10))
    ...
    if args.sample == 'lowz':
        ...
        tbl['r200'] \
            = (3*1e14*tbl['mass']*u.Msun \
                 / (800*np.pi*cosmo.critical_density(tbl['z'])))**(1/3)
    ...
    if splus:
        ...
        if args.sample == 'lowz':
            ...
            for i, (m, c, s) \
                    in enumerate(zip(
                        sample_masks, ('C3', 'C3', 'C2'), (25, 35, 25))):
                kwargs['c'] = c
                kwargs['s'] = s
                if i >= 1:
                    kwargs['label'] = '_none_'
    ...
    if matching is not None:
        ...
        for (cname, mcat, maxsep), marker in zip(matching, 'osx^v<>8D'):
            ...
            if cname == 'WINGS':
                ...
                lines.append(
                    axes[1].plot(
                        ...
                        f'C1{marker}', **kwargs)[0])
            if cname != 'CHANCES':
                continue
            lines.append(axes[1].plot(
                tbl['z'][matches[cname]], tbl['mass'][matches[cname]],
                f'k{marker}', **kwargs)[0])
            axes[2].plot(
            ...
            mra[mra > 180] -= 360
            axes[2].plot(
                mra[missing], mcat['dec'][missing], 'kD',
                ms=10, mfc='none', label='Additional CHANCES')
...
def supercluster_mask(ra=None, dec=None, what='masks', shapley_kwargs={},
                      horologium_kwargs={}, wrap_ra=180):
    ...
    if wrap_ra != 0:
        ...
        ra_horo = [[xi-2*wrap_ra if xi > wrap_ra else xi for xi in x]
                   for x in ra_horo]
    if what == 'masks':
        shapley = (ra > ra_shapley[0]) & (ra < ra_shapley[1]) \
            & (dec > dec_shapley[0]) & (dec < dec_shapley[1])
        horologium = np.array(np.max(
            [(ra > ra_i[0]) & (ra < ra_i[1]) & (dec > dec_i[0]) & (dec < dec_i[1])
            for ra_i, dec_i in zip(ra_horo, dec_horo)], axis=0), dtype=bool)
    elif what == 'lines':
        shapley = [
        ...
        shapley = [
            Rectangle(
                (ra_shapley[0], dec_shapley[0]), ra_shapley[1]-ra_shapley[0],
                dec_shapley[1]-dec_shapley[0], **shapley_kwargs)]
        horologium = [
            Rectangle(
                (ra_i[0], dec_i[0]), ra_i[1]-ra_i[0], dec_i[1]-dec_i[0],
                **horologium_kwargs)
            for ra_i, dec_i in zip(ra_horo, dec_horo)]
